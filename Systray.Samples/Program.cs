...
static void ShowContextMenu(HWND hwnd, PhysicalPoint pt, bool isRightClick, ILogger logger)
{
    ...
    MenuHelpers.InsertMenuItem(submenu, 0, new MenuItemInfo { Text = "Welcome to the submenu!", Enabled = false });
    MenuHelpers.InsertMenuItem(submenu, 1, MenuItemInfo.CreateSeparator());
    MenuHelpers.InsertMenuItem(submenu, 2, new MenuItemInfo { Text = "&Checked option", Id = 102, Checked = true });
    MenuHelpers.InsertMenuItem(submenu, 3, new MenuItemInfo { Text = "&Highlighted option", Id = 103, Hilite = true });
    MenuHelpers.InsertMenuItem(submenu, 4, new MenuItemInfo { Text = "&Default option", Id = 104, Default = true });

    using var menu = MenuHelpers.CreatePopupMenu();

    uint exitId = 1;
    MenuHelpers.InsertMenuItem(menu, 0, new MenuItemInfo { Text = "My app", Enabled = false });
    MenuHelpers.InsertMenuItem(menu, 1, new MenuItemInfo { Text = "&Options", SubMenu = submenu });
